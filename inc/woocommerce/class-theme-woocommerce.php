...
if ( ! class_exists( 'Flintotheme_WooCommerce' ) ) :

/**
 * The Flintotheme WooCommerce Integration class
 */
class Flintotheme_WooCommerce {
	...
	public function __construct() {
      	add_filter('flintotheme_navigation_class', array($this, 'add_class_posts_navigation'));
		add_filter('woocommerce_locate_template', array($this, 'custom_woocommerce_locate_template'), 10, 3);
		add_filter('flintotheme_heading_bar_display_data', array($this, 'heading_bar_display_data'), 10, 3);
		add_filter('flintotheme_ordering_comment_field', array($this, 'ordering_comment_field'), 10);

		// product image size
		add_filter( 'woocommerce_get_image_size_thumbnail', 'flintotheme_woocommerce_custom_image_size_thumbnail' );
		add_filter( 'woocommerce_get_image_size_single', 'flintotheme_woocommerce_custom_image_size_single' );
		add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'flintotheme_woocommerce_custom_image_size_gallery_thumbnail' );
    ...
    public function prdctfltr_before_ajax_json_send($data) {
		$args = apply_filters('flintotheme_woo_pagination_args', array(
			'type' 	    => 'list',
	...
	function move_quickview_button_in_loop_product() {
		...
		add_action('flintotheme_woo_after_loop_thumbnail_link', array($WC_Quick_View, 'quick_view_button'), 15);
	}
	...
	function add_bwc_product_quickview_button_in_loop_product() {
		global $BWC_Product_Quick_View;
		add_action('flintotheme_woo_after_loop_thumbnail_link', array($BWC_Product_Quick_View, 'quick_view_button'), 15);
	}
...
return new Flintotheme_WooCommerce();
