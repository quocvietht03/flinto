 ...
 *
 */
class Flintotheme_Footer_Builder_Control extends WP_Customize_Control {

	public function render_content() {
  ?>
  <div id="theme-extends-footer-action-control">
    <div class="theme-extends-customize-control-title"><?php _e( 'Footer Customizer', 'flintotheme' ) ?></div>
    <div class="theme-extends-message">
      <span class="ion-information-circled"></span>
      <?php _e('The Footer Customizer allows you to creative footer layout with drag-drop the components.', 'flintotheme'); ?>
    </div>
          ...
          <div class="layout-content">
            <div
              ...
              <div v-if="footer_edit == item" class="editing-overlay">
                <span><?php _e('Editing...', 'flintotheme'); ?><span>
              </div>
              <div class="title">{{ item.name }}</div>
              <customize-builder :data="item.footer_data" mode="preview"></customize-builder>
            </div>
            <div class="actions">
              <div class="item-action edit-action" @click="edit_layout_handle($event, item)" title="<?php esc_attr_e('edit', 'flintotheme'); ?>"><span class="ion-gear-a"></span></div>
              <div v-if="(! item.lock || item.lock == false)" @click="remove_layout_handle($event, item, index)" class="item-action remove-action" title="<?php esc_attr_e('remove', 'flintotheme'); ?>"><span class="ion-android-delete"></span></div>
            </div>
          </div>
        </el-col>
          ...
          <div class="layout-content">
            <div class="add-footer-layout" @click="new_layout_handle($event)">
              <div class="add-footer-layout-inner">
                <span class="ion-plus-round"></span> <?php _e('New layout', 'flintotheme'); ?>
              </div>
            </div>
          </div>
        </el-col>
