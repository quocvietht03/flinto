...
if(! class_exists('Flintotheme_Header_Render')) :
  /**
   * @since 1.0.0
   * Header builder render
   * extends Flintotheme_Customize_Builder_Render  
   */
  class Flintotheme_Header_Render extends Flintotheme_Customize_Builder_Render {
    ...
    function render() { 
      ...
      <div class="header-builder-wrap <?php echo esc_attr( $this->header_key ); ?>" data-header-settings='<?php echo esc_attr($header_settings); ?>'>
        <div class="header-builder__main" data-design-name="<?php esc_attr_e('Header Main', 'flintotheme'); ?>" data-design-selector="#page .<?php echo esc_attr( $this->header_key ); ?> .header-builder__main"><?php echo "{$header_desktop}"; ?></div>
        <?php if($this->general_settings['header_sticky'] == true) { ?>
          <div class="header-builder__sticky" data-design-name="<?php esc_attr_e('Header Sticky', 'flintotheme'); ?>" data-design-selector="#page .<?php echo esc_attr( $this->header_key ); ?> .header-builder__sticky"><?php echo "{$header_sticky}"; ?></div>
        <?php } ?>
        <div class="header-builder__mobile" data-design-name="<?php esc_attr_e('Header Sticky', 'flintotheme'); ?>" data-design-selector="#page .<?php echo esc_attr( $this->header_key ); ?> .header-builder__mobile"><?php echo "{$header_tablet_mobile}"; ?></div>
      </div>
      <?php
    }
