...
class Flintotheme_Installer_Helper {
	public static function download_and_install_a_package( $package, $destination, $hook_extra = array(), $folder = false ) {
		 ...
		 */
		$upgrader = new WP_Upgrader( new Flintotheme_Auto_Install_Upgrader_Skin() );
...
class Flintotheme_Plugin_Installer_Helper extends Flintotheme_Installer_Helper {
	/**
	...
	public static function install( $plugin, $force = false ) {
		...
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'flintotheme' )
				)
			)
	...
	public static function activate( $plugin ) {
		...
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'flintotheme' ), $plugin['name'] )
				)
			);
		}
	...
	public static function bulk_activate( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'flintotheme' ), WP_PLUGIN_DIR ),
			);
		}
		$status = array();
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'flintotheme' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'flintotheme' ), wp_remote_retrieve_body( $request ) );
				}
			}
	...
	public static function is_active( $plugin ) {
		...
		if( function_exists('flintotheme_check_plugin_is_active') ) {
			return ! self::is_installed( $plugin ) ? false : flintotheme_check_plugin_is_active( self::get_name( $plugin['slug'] ) );
		} else {
			return true;
		}
	}
...
class Flintotheme_Auto_Install_Upgrader_Skin extends WP_Upgrader_Skin {
