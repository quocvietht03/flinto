...
if(! class_exists('Flintotheme_Customize_Builder_Render')) :
  class Flintotheme_Customize_Builder_Render {
    public $footer_key, $layout_data;

    public static $element_maps = array(
      'rs-row'      => 'flintotheme_rs_row_element',
      'rs-col'      => 'flintotheme_rs_col_element',
      'widget'      => 'flintotheme_widget_element',
      'wp-widget'   => 'flintotheme_wp_widget_element',
    );
    ...
    public static function build_output($layout_data) {
      ...
      foreach($layout_data as $item) :
        $_fn = Flintotheme_Customize_Builder_Render::$element_maps[$item['element']];
        ...
        $output .= implode('', array(
          ...
          (isset($item['children']) && count($item['children']) > 0)
            ? Flintotheme_Customize_Builder_Render::build_output($item['children'])
            : '',
    ...
    public static function build_style_inline($layout_data = array()) {
      ...
      foreach($layout_data as $item) :
        ...
        $output .= Flintotheme_Customize_Builder_Render::build_style_responsive_inline($class_name, $params);
      
        if(isset($item['children']) && count($item['children']) > 0) {
          $output .= Flintotheme_Customize_Builder_Render::build_style_inline($item['children']);
        }
      endforeach;
