 ...
 */
class Flintotheme_Widget_Recent_Posts extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_recent_posts',
            __('Theme Widget - Recent Posts', 'flintotheme'),
            __('Your site’s most recent Posts.', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )->set_default_value( __('Recent Posts', 'flintotheme') ) ,
                Field::make( 'text', 'number_posts_show', __('Number of posts to show', 'flintotheme') )
                    ->set_attribute( 'type', 'number' )
                    ->set_attribute( 'min', 1 )
                    ->set_attribute( 'max', 20 )
                    ->set_default_value( 5 ),
                Field::make( 'checkbox', 'display_post_date', __('Display post date?', 'flintotheme') )
                    ->set_default_value( true ),
                Field::make( 'checkbox', 'display_post_author', __('Display post author?', 'flintotheme') )
                    ->set_default_value( true ),
            ...
            <div class="__inner">
                <?php if( ! empty($recent_posts) && count($recent_posts) > 0 ) {
                    ...
                    foreach( $recent_posts as $_p ) {
                        ...
                        $post_by = '<div class="p-author">' . __('by', 'flintotheme') . ' ' . '<a href="'. esc_url( get_author_posts_url($_p['post_author']) ) .'">' . get_the_author_meta('user_nicename', $_p['post_author']) . '</a>' . '</div>';
                    ...
                    ?>
                    <p><?php _e('There are no posts to display..!', 'flintotheme') ?></p>
                    <?php
                }
                wp_reset_query();
                ?>
