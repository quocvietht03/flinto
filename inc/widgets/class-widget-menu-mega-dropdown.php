 ...
 */
class Flintotheme_Widget_Menu_Mega_Dropdown extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_Menu_mega_dropdown',
            __('Theme Widget - Menu Mega Dropdown', 'flintotheme'),
            __('Displays menu mega dropdown', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )
                    ->set_default_value( __('Menu Mega Dropdown', 'flintotheme') ),
                Field::make( 'select', 'layout', __( 'Layout', 'flintotheme' ) )
                    ->add_options( apply_filters( 'flintotheme_widget_menu_mega_dropdown_layout_filter', array(
                        'default' => __('Default', 'flintotheme'),
                        'minimal' => __('Minimal', 'flintotheme')
                    ) ) )
                    ->set_default_value( 'default' ),
                Field::make( 'text', 'select_name', __('Label', 'flintotheme') )
                    ->set_default_value( __('Quick Links', 'flintotheme') ),
                Field::make( 'select', 'open_menu_on_event', __( 'Open Menu On', 'flintotheme' ) )
                    ->add_options( array(
                        'hover' => 'Mouse Hover',
                        'click' => 'Mouse Click',
                    ) )
                    ->set_default_value( 'hover' ),
                Field::make( 'checkbox', 'menu_open', __( 'Menu Open', 'flintotheme' ) )
                ...
                Field::make( 'complex', 'menu_data' )
                    ...
                    ->add_fields( array(
                        Field::make( 'text', 'label', __( 'Label', 'flintotheme' ) )
                            ->set_default_value( __( 'Menu Item', 'flintotheme' ) ),
                        Field::make( 'select', 'icon_type', __('Icon Type', 'flintotheme') )
                            ->add_options( array(
                                'icon_image' => __( 'Icon Image', 'flintotheme' ),
                                'icon_font' => __( 'Icon Font', 'flintotheme' ),
                            ) )
                            ->set_default_value( 'image' ),
                        Field::make( 'text', 'icon_font', __( 'Icon Font — Classes', 'flintotheme' ) )
                            ->set_conditional_logic( array(
                                array(
                                    'field' => 'icon_type',
                                    'value' => 'icon_font',
                        ...
                        Field::make( 'image', 'icon_image', __( 'Select Image', 'flintotheme' ) )
                            ->set_value_type( 'url' )
                            ->set_conditional_logic( array(
                                array(
                                    'field' => 'icon_type',
                        ...
                        Field::make( 'text', 'url', __( 'Url', 'flintotheme' ) )
                            ->set_default_value( '#' ),
                        Field::make( 'select', 'menu_style', __('Menu Style', 'flintotheme') )
                            ->add_options( array(
                                'default' => __( 'Default', 'flintotheme' ),
                                // 'menu_inner' => __( 'Menu Inner', 'flintotheme' ),
                                'menu_mega_multiple_columns' => __( 'Menu Mega Multiple Columns — Light', 'flintotheme' ),
                                'menu_mega_multiple_columns_dark' => __( 'Menu Mega Multiple Columns — Dark', 'flintotheme' ),
                            ) )
                            ->set_default_value( 'menu_inner' ),
                        Field::make( 'select', 'menu_inner', __('Sub — Menu', 'flintotheme') )
                        ...
                        Field::make( 'image', 'menu_item_background_image', __( 'Sub — Background Image', 'flintotheme' ) )
                            ->set_conditional_logic( array(
                                array(
                                    'field' => 'menu_style',
                        ...
                        Field::make( 'select', 'menu_mega_columns', __('Sub — Menu Mega Columns', 'flintotheme') )
                            ->add_options( array(
                                2 => __( '2 Columns', 'flintotheme' ),
                                3 => __( '3 Columns', 'flintotheme' ),
                                4 => __( '4 Columns', 'flintotheme' ),
                            ) )
                            ->set_default_value( 4 )
                            ->set_conditional_logic( array(
                                array(
                        ...
                        Field::make( 'complex', 'menu_mega', __( 'Sub — Menu Mega Data', 'flintotheme' ) )
                            ->set_collapsed( true )
                            ->set_conditional_logic( array(
                                array(
                                    'field' => 'menu_style',
                            ...
                            ->add_fields( array(
                                Field::make( 'image', 'menu_item_image', __( 'Menu Item Image', 'flintotheme' ) ),
                                Field::make( 'select', 'menu_inner', __('Menu', 'flintotheme') )
                                    ->add_options( 'flintotheme_widget_get_list_menu_options' )
                                    ->set_default_value('')
                                    ->set_conditional_logic( array(
                                        array(
    ...
    public function _render_menu_options($instance, $echo = true) {
        ...
        foreach( $menu_data as $item ) {
            $icon_temp = array(
                'icon_image' => ! empty( $item['icon_image'] ) ? '<img class="__menu-icon __icon-image" src="'. esc_url( $item['icon_image'] ) .'" alt="'. esc_attr( 'image', 'flintotheme' ) .'">' : '',
                'icon_font' => ! empty( $item['icon_font'] ) ? '<span class="__menu-icon __icon-font"><i class="'. esc_attr( $item['icon_font'] ) .'"></i></span>' : '',
            );

            ob_start();
            /**
             * hook flintotheme_widget_menu_mega_dropdown_submenu_html. 
             * 
             * @see flintotheme_widget_menu_mega_dropdown_submenu_default - 20 
             */
            do_action( 'flintotheme_widget_menu_mega_dropdown_submenu_html', $item, $instance ); 
    ...
    function front_end( $args, $instance ) {
        ...
        if(! empty($instance['class_custom_design'])) {
            ...
            $custom_design_selector = array(
                array(
                    'name' => __('Widget menu mega dropdown wrap', 'flintotheme'),
                    'selector' => $_root_classes
                ),
            );

            $custom_design_name_attr = 'data-design-name="'. __('Widget Menu Mega Dropdown', 'flintotheme') .'"';
            $custom_design_selector_attr = 'data-design-selector=\''. wp_json_encode( $custom_design_selector ) .'\'';
        }

        $layouts = apply_filters( 'flintotheme_widget_menu_mega_dropdown_layout_filter', array(
...
if(! function_exists('flintotheme_widget_menu_mega_dropdown_submenu_default')) {
    /**
     * @since 1.0.0
     * 
     * @param [array] $data
     ...
     */
    function flintotheme_widget_menu_mega_dropdown_submenu_default($data, $instance) {
        ...
        $classes = apply_filters( 'flintotheme_widget_menu_mega_dropdown_submenu_default_classes', array(
            '__sub-menu', 
    ...
    add_action( 'flintotheme_widget_menu_mega_dropdown_submenu_html', 'flintotheme_widget_menu_mega_dropdown_submenu_default', 20, 2 );
}

if(! function_exists('flintotheme_widget_menu_mega_dropdown_submenu_multiple_columns')) {
    /**
     * @since 1.0.0
     *  
     * @param [array] $data
     * @param [array] $instance
     */
    function flintotheme_widget_menu_mega_dropdown_submenu_multiple_columns( $data, $instance ) {
    ...
    add_action( 'flintotheme_widget_menu_mega_dropdown_submenu_html', 'flintotheme_widget_menu_mega_dropdown_submenu_multiple_columns', 20, 2 );
}
