 ...
 */
class Flintotheme_Widget_Featured_Box_Slide extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_featured_box_slide',
            __('Theme Widget - Featured Box Slide', 'flintotheme'),
            __('Displays featured box slide', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )->set_default_value( __('Our Services', 'flintotheme') ),
                Field::make( 'complex', 'featured_box_slide_data', 'Images Slide' )
                ->set_layout( 'tabbed-horizontal' )
                ->add_fields( array(
                    Field::make( 'select', 'icon_type', 'Text alignment' )
                        ->add_options( array(
                            'icon_font' => __('Icon Font - Classes', 'flintotheme'),
                            'image' => 'Image from Gallery',
                        ) )
                        ->set_default_value( 'icon_font' )
                        ->set_help_text( __('Note: for Icon Font type you can use Fontawesome 4.x, Ionicon, Dashion.', 'flintotheme') ),
                    Field::make( 'image', 'icon_image', __('Icon', 'flintotheme') )
                    ...
                    Field::make( 'text', 'icon_font', __('Icon', 'flintotheme') )
                        ->set_conditional_logic( array(
                            array(
                                'field' => 'icon_type',
                                'value' => 'icon_font',
                    ...
                    Field::make( 'text', 'name', __('Name', 'flintotheme') ),
                    Field::make( 'rich_text', 'descriptions', __('Descriptions', 'flintotheme') ),
                ) ),
    ...
    function _render_icon_html($args = array()) {
        ...
        $template = array(
            ...
            'image' => '<div class="fbox-icon icon-type-icon_image"><img src="'. esc_url( $image_data[0] ) .'" alt="'. esc_attr('image', 'flintotheme') .'"></div>',
        );
    ...
    function front_end( $args, $instance ) { 
        ...
        if(! empty($instance['class_custom_design'])) {
            ...
            $custom_design_selector = wp_json_encode( array(
                array(
                    'name' => __('Featured box item wrap', 'flintotheme'),
                    'selector' => $_root_classes . ' .fbox-item .__inner',
                ),
                array(
                    'name' => __('Featured box - Icon', 'flintotheme'),
                    'selector' => $_root_classes . ' .fbox-item .__inner .fbox-icon',
                ),
                array(
                    'name' => __('Featured box - Title', 'flintotheme'),
                    'selector' => $_root_classes . ' .fbox-item .__inner .fbox-entry .fbox-title',
                ),
                array(
                    'name' => __('Featured box - Descriptions', 'flintotheme'),
                    'selector' => $_root_classes . ' .fbox-item .__inner .fbox-entry .fbox-des',
                ),
            ) );

            $custom_design_name_attr = 'data-design-name="'. __('Featured Box', 'flintotheme') .'"';
        ...
        <div 
            ...
            <?php echo apply_filters( 'flintotheme_widget_featured_box_design_name_attr', $custom_design_name_attr ); ?> 
            <?php echo apply_filters( 'flintotheme_widget_featured_box_design_selector_attr', $custom_design_selector_attr ); ?>>
