 ...
 */
class Flintotheme_Widget_Product_Custom_Search extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_product_custom_search',
            __('Theme Widget - Product Search', 'flintotheme'),
            __('Displays a search field search products', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )
                    ->set_default_value( __('Products Search', 'flintotheme') ),
                Field::make( 'checkbox', 'suggestions_product', __('On/Off Suggestions Product', 'flintotheme') )
                ...
                Field::make( 'textarea', 'suggestions_product_short_desc', 'Suggestions Product Short Descriptions' )
                    ...
                    ->set_default_value( __('Our featured products', 'flintotheme') ),
                Field::make( 'select', 'layout', __( 'Layout', 'flintotheme' ) )
                    ->add_options( apply_filters( 'flintotheme_widget_product_custom_search_layout_filter', array(
                        'default' => __('Default', 'flintotheme'),
                        'popup' => __('Popup', 'flintotheme'),
                    ) ) )
                    ->set_default_value( 'default' ),
                Field::make( "multiselect", "suggestions_product_data", __("Suggestions Products", 'flintotheme') )
                    ->set_conditional_logic( array(
                        array(
                            'field' => 'suggestions_product',
                            'value' => true,
                    ...
                    ->add_options( flintotheme_woo_widget_get_product_options( array( '' => __('- Select Product -', 'flintotheme') ) ) ),
    ...
    public function _template($temp = 'default') {
        $template = array(
            ...
            'popup' => $this->flintotheme_widget_product_custom_search_layout_popup(),
        );
        $template = apply_filters( 'flintotheme_widget_product_custom_search_layouts', $template );
        return $template[$temp];
    }

    public function flintotheme_widget_product_custom_search_layout_popup(){
        $layout = implode('', array(
            '<div class="_icon-search">'. flintotheme_search_icon_svg( 'search_basic' ) .'</div>',
            '<div class="theme-extends-product-custom-search-wrapper theme-extends-product-custom-search-ajax-js">',
        			'<div class="product-custom-search-container">',
        				'<div class="search-form-container">',
        					'<a href="javascript:" class="__close" title="'. esc_attr__( 'Close', 'flintotheme' ) .'">×</a>',
                  '<form class="theme-extends-custom-product-search woocommerce-product-search" method="get" action="'. get_site_url() .'" role="search" data-woo-widget-custom-product-search-form>',
                      '<input type="search" class="search-field product-search-field" placeholder="{{placeholder_text}}" value="" name="s" autocomplete="off">',
                      '{{by_category_filter_html}}',
                      '<button type="submit" value="Search">{{button_submit_text}}</button>',
    ...
    public function category_filter_html() {
        ...
        array_push($output, '<label><span class="__cat_label_display">'. __('Select Category', 'flintotheme') .'</span></label>');
        array_push($output, '<ul class="p-cat-list">');
        array_push($output, '<li class="_p-all"><a href="#" data-pcat-slug="" data-pcat-name="'. esc_attr__('Select Category', 'flintotheme') .'">'. __('Select Category', 'flintotheme') .'</a></li>');
    ...
    public function suggestions_product_html( $short_descriptions = '', $product_data = array() ) {
        ...
        foreach($posts as $p) {
            ...
            array_push($result_products, flintotheme_woo_product_item_widget_search_temp($product));
        }
    ...
    function front_end( $args, $instance ) {
        ...
        <div
            ...
            <?php echo apply_filters( 'flintotheme_widget_product_search_form_design_name_attr', $custom_design_name_attr ); ?>
            <?php echo apply_filters( 'flintotheme_widget_product_search_form_design_selector_attr', $custom_design_selector_attr ); ?>>
            <div class="__inner">
