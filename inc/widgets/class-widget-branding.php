 ...
 */
class Flintotheme_Widget_Branding extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_branding',
            __('Theme Widget - Branding Gallery', 'flintotheme'),
            __('Displays a grid brading gallery 1-4 columns', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )->set_default_value( __('Branding', 'flintotheme') ) ,
                Field::make( 'complex', 'branding_gallery_data', 'Branding Gallery' )
                ->set_layout( 'tabbed-horizontal' )
                ->add_fields( array(
                    Field::make( 'image', 'image', __('Image', 'flintotheme') ),
                    Field::make( 'text', 'name', __('Name', 'flintotheme') ),
                    Field::make( 'text', 'url', __('Url', 'flintotheme') ),
                ) ),
                Field::make( 'select', 'branding_gallery_column', __('Column(s)', 'flintotheme') )
                ->add_options( array(
                    '1' => __('1 Column', 'flintotheme'),
                    '2' => __('2 Columns', 'flintotheme'),
                    '3' => __('3 Columns', 'flintotheme'),
                    '4' => __('4 Columns', 'flintotheme'),
                ) )
                ->set_default_value( '2' ),
                Field::make( 'checkbox', 'open_on_new_window', __('Open on new window', 'flintotheme') ),
        ) );
    }
