 ...
 */
class Flintotheme_Widget_Wishlist_Products_Counter extends Flintotheme_Widget_Abstract {

	// Register widget function. Must have the same name as the class
    function __construct() {
        $this->setup(
            'theme_widget_wishlist_products_counter',
            __('Theme Widget - Wishlist Products Counter', 'flintotheme'),
            __('Displays the number of products in the wishlist on your site.', 'flintotheme'),
            array(
                Field::make( 'text', 'title', __('Title', 'flintotheme') )
                    ->set_default_value( __('Wishlist', 'flintotheme') ),
                Field::make( 'select', 'layout', __( 'Layout', 'flintotheme' ) )
                    ->add_options( apply_filters( 'flintotheme_widget_wishlist_products_counter_layout_filter', array(
    ...
    public function _render_layout_default($instance) {
        ...
        <div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
            <a href="<?php echo esc_url( $page_wishlist_link ); ?>">
                <div class="icon-entry" data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_attr('My Wishlist', 'flintotheme'); ?>">
                    <div class="_icon-wishlist">
                        <?php echo flintotheme_wishlist_icon_svg( 'default' ); ?>
                    </div>
                    <span class="theme_extends_wishlist_products_counter_number __empty" data-design-name="<?php _e( 'Wishlist counter', 'flintotheme' ); ?>" data-design-selector="#page .theme-extends-widget-wishlist-products-counter .theme_extends_wishlist_products_counter_number">0</span>
                </div>
            </a>
        </div>
		<?php
    ...
    function front_end( $args, $instance ) {
        ...
        if(! empty($instance['class_custom_design'])) {
            ...
            $custom_design_selector = array(
                array(
                    'name' => __('Widget wishlist counter wrap', 'flintotheme'),
                    'selector' => $_root_classes
                ),
                // array(
                //     'name' => __('Widget wishlist counter content', 'flintotheme'),
            ...
            if( in_array( $instance['layout'], array( 'default' ) ) ) {
                array_push( $custom_design_selector, array(
                    'name' => __( 'SVG Icon Wishlist', 'flintotheme' ),
                    'selector' => $_root_classes . ' .icon-entry svg',
                ) );
            }

            $custom_design_name_attr = 'data-design-name="'. __('Widget wishlist counter', 'flintotheme') .'"';
            $custom_design_selector_attr = 'data-design-selector=\''. wp_json_encode( $custom_design_selector ) .'\'';
        }

        $layouts = apply_filters( 'flintotheme_widget_custom_wishlist_products_counter_filter', array(
