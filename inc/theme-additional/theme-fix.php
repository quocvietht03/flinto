 ...
 * Flintotheme 
 */ 
add_action( 'add_meta_boxes', 'flintotheme_custom_metabox_func' );
...
if( class_exists('WooCommerce') ){
    add_action( 'add_meta_boxes', 'flintotheme_woo_add_meta_boxes_hooks' );
}

/*============ Functions ==============*/

if(! function_exists('flintotheme_custom_metabox_func')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_custom_metabox_func($post_type) {
        global $flintotheme;
        $list_metabox = $flintotheme->metabox->get_list_metabox();

        if( ! in_array( $post_type . '.php', $list_metabox ) ) return;

        $metabox_title = sprintf( '%1$s %2$s', $post_type, __('Options', 'flintotheme') );
        add_meta_box(
            'fintotheme_custom_metabox_' . $post_type,
            apply_filters('fintotheme_custom_metabox_' . $post_type, $metabox_title, $post_type ),
            array( 'Flintotheme_Custom_Meta_Box', 'render_metabox' ),
            $post_type,
            'advanced',
            'high'
        );
...
if(! function_exists('flintotheme_woo_add_meta_boxes_hooks')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_woo_add_meta_boxes_hooks() {
        global $post;
        ...
        if( get_post_type($post) == 'product' ) {
            ...
            add_action( 'flintotheme_metabox_customize_settings_after_general', 'flintotheme_metabox_customize_product_detail_settings_panel', 20 );
        }
    }
}

if(! function_exists('flintotheme_check_plugin_is_active')) {
    /**
     * @since 1.0.0
     * is_plugin_active
     */
    function flintotheme_check_plugin_is_active($plugin) {
        return is_plugin_active( $plugin );
    }
}

if(! function_exists('flintotheme_return_server_software')) {
    /**
     * @since 1.0.0 
     */
    function flintotheme_return_server_software() {
        return $_SERVER['SERVER_SOFTWARE'];
    }
}

if(! function_exists('Flintotheme_Jetpack_Portfolio_Add_Meta_Boxed')) {
    /**
     * @since 1.0.0
     *  
     */
    function Flintotheme_Jetpack_Portfolio_Add_Meta_Boxed() {
        add_action( 'add_meta_boxes', 'Flintotheme_Jetpack_Portfolio_Metabox_Hooks_Manager' );
    }
}

add_action( 'add_meta_boxes', 'flintotheme_remove_heading_bar_setting_is_blog_page', 20, 2 );

if(! function_exists('flintotheme_select_menu_message_fallback_template')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_select_menu_message_fallback_template() {
        echo '<div class="theme-extends-message-menu-fallback"><div class="theme-extends-menu-message">' . esc_html__( 'Please go to the', 'flintotheme' ) . ' <a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" target="_blank">' . esc_html__( 'Menu', 'flintotheme' ) . ' <i class="fa fa-external-link" aria-hidden="true"></i></a> ' . esc_html__( 'section, create a  menu and then select the newly created menu from the Theme Locations box from the left.', 'flintotheme' ) . '</div></div>';
    }
}
add_action( 'flintotheme_select_menu_message_fallback_action', 'flintotheme_select_menu_message_fallback_template', 20 );

if(! function_exists('flintotheme_verify_purchase_code_form_activate_html')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_verify_purchase_code_form_activate_html( $echo = true ) {
        ob_start();
        ?>
        ...
        <form class="theme-verify-purchase-code-form" method="POST">
            ...
            <p class="__text_note">
                In order to receive all benefits of Flintotheme, you need to activate your copy of the theme. By activating Flintotheme license you will unlock premium options - direct theme <u>updates</u>, access to <u>premium plugin library</u> and one click <u>install package demo</u>.
            </p>
            <p class="__text_note">
...
if(! function_exists('flintotheme_verify_purchase_code_form_deactivate_html')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_verify_purchase_code_form_deactivate_html( $echo = true ) {
        ob_start();
        ?>
...
if(! function_exists('flintotheme_verify_purchase_code_form_html')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_verify_purchase_code_form_html() {
        $verifytheme_settings = get_option( 'verifytheme_settings' , array( 'purchase_code' => '' ) );
        $theme_verify = ( empty( $verifytheme_settings['purchase_code'] ) ) ? false : isInstallationLegit( $verifytheme_settings );
        ?>
        <h3><?php _e('Theme Activation License', 'flintotheme'); ?></h3>
        <div id="theme-purchase-code-validate-js" class="theme-purchase-code-validate">
            <?php 
            if( true == $theme_verify ) {
                flintotheme_verify_purchase_code_form_deactivate_html();
            } else {
                flintotheme_verify_purchase_code_form_activate_html();
            }
            ?>
        </div>
        <?php
...
add_action( 'flintotheme_verify_purchase_code_form' , 'flintotheme_verify_purchase_code_form_html', 10 );

if( ! function_exists('flintotheme_ajax_purchase_code_validate') ) {
    /**
     * @since 10.0
     *  
     */
    function flintotheme_ajax_purchase_code_validate() {
        $VerifyTheme = new VerifyTheme();
        $result = $VerifyTheme->main_purchase_code_validate( $_POST['purchase_code'] );

        $result['content'] = flintotheme_verify_purchase_code_form_deactivate_html( false );
        wp_send_json_success( $result );
        exit();
    }

    add_action( 'wp_ajax_flintotheme_ajax_purchase_code_validate', 'flintotheme_ajax_purchase_code_validate' );
    add_action( 'wp_ajax_nopriv_flintotheme_ajax_purchase_code_validate', 'flintotheme_ajax_purchase_code_validate' );
}

if( ! function_exists('flintotheme_ajax_deactive_license_theme') ) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_ajax_deactive_license_theme() {
        ...
        wp_send_json_success( array(
            ...
            'content' => flintotheme_verify_purchase_code_form_activate_html( false ),
            'st' => 'success',
        ) );
        exit();
    }

    add_action( 'wp_ajax_flintotheme_ajax_deactive_license_theme', 'flintotheme_ajax_deactive_license_theme' );
    add_action( 'wp_ajax_nopriv_flintotheme_ajax_deactive_license_theme', 'flintotheme_ajax_deactive_license_theme' );
}

if(! function_exists('flintotheme_header_builder_custom_layout_data_default')) {
    /**
     * @since 1.0.0
     *  
     */
    function flintotheme_header_builder_custom_layout_data_default() {
    ...
    add_filter( 'flintotheme_header_builder_layout_data_default' , 'flintotheme_header_builder_custom_layout_data_default' );
}