...
! (function(w, d, $) {
  ...
  var themeScriptApi = function() {
      ...
      $el.on({
        ...
        '_loadData.search-form' (e, data, callback) {
          $.ajax({
            ...
            data: {action: 'flintotheme_search_form_ajax_load_data', s: data},
            success (result) {
        ...
        $form.on({
          ...
          'submit'(e) {
            ...
            $.ajax({
              type: 'POST',
              url: theme_script_object.ajax_url,
              data: 'action=flintotheme_delipress_submit_custom_form_handle&' + data,
              success(res) {
                if(res.success == true) {
    ...
    this.likePost = function() {
      ...
      $('body').on('click', '[data-themeextends-button-like-post]', function(e) {
        ...
        $.ajax({
          type: 'POST',
          url: theme_script_object.ajax_url,
          data: { action: 'flintotheme_post_likes_handle', data: { pid: pid, inc: increase } },
          success (res) {
    ...
    this.smoothScrollbar = function() {
      ...
      $('.flintotheme-archive-filter-tool-bar.__layout-default .widget .__widget-inner > *:not(.widget-title)').each(function() {
          ...
          $formEl.on({
            ...
            '__on_search'(e, s) {
              ...
              $.ajax({
                type: 'POST',
                url: theme_script_object.ajax_url,
                data: 'action=flintotheme_woo_widget_product_search_func&' + data,
                success(result) {
      ...
      $('[data-woo-product-gallery-trigger2]').on({
        ...
        '__g-get-image' (e, image_id) {
          ...
          $.ajax({
            type: 'POST',
            url: theme_script_object.ajax_url,
            data: { action: 'flintotheme_woo_ajax_get_image_by_id', att_id: image_id },
            success(res) {
